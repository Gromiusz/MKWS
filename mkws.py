...
for counter in range(8):
    ...
    inlet_open = (-3. - counter*5.) / 180. * np.pi
    inlet_close = (183. + counter*5.) / 180. * np.pi

    # Outlet valve friction coefficient, open and close timings
    outlet_valve_coeff = 1.e-6
    outlet_open = (537. - counter*5.) / 180 * np.pi
    outlet_close = (3. + counter*5.) / 180. * np.pi
    ...
    # heat release
    Q[counter] = trapz(states.heat_release_rate * states.V, t)
    output_str = '{:45s}{:>4.1f} {}'
    print(output_str.format('Heat release rate per cylinder (estimate):',
                            Q[counter] / t[-1] / 1000., 'kW'))
    Q1[counter] = Q[counter] / t[-1] / 1000.

    # expansion power
    W[counter] = trapz(states.dWv_dt, t)
    print(output_str.format('Expansion power per cylinder (estimate):',
                            W[counter] / t[-1] / 1000., 'kW'))
    W1[counter] = W[counter] / t[-1] / 1000.

    # efficiency
    eta[counter] = W[counter] / Q[counter]
    print(output_str.format('Efficiency (estimate):', eta[counter] * 100., '%'))

    # CO emissions
    MW[counter] = states.mean_molecular_weight
    CO_emission[counter] = trapz(MW[counter] * states.mdot_out * states('CO').X[:, 0], t)
    CO_emission[counter] /= trapz(MW[counter] * states.mdot_out, t)
    print(output_str.format('CO emission (estimate):', CO_emission[counter] * 1.e6, 'ppm'))


a = [1,2,3,4,5,6,7,8]
fig, ax = plt.subplots()
ax.plot(a, Q1)
ax.set_xlabel('Ordinal number of valve timing case')
ax.set_ylabel('Heat release rate per cylinder [kW]')
plt.show()

fig, ax = plt.subplots()
ax.plot(a, W1)
ax.set_xlabel('Ordinal number of valve timing case')
ax.set_ylabel('Expansion power per cylinder [kW]')
plt.show()
...
fig, ax = plt.subplots()
ax.plot(a, CO_emission)
ax.set_xlabel('Ordinal number of valve timing case')
ax.set_ylabel('CO emission [ppm]')
plt.show()